...
export function Graph({ data }: GraphProps) {
  ...
  let tow = data.points?.find((point) => point.name == "TOW")
  let elw = data.points?.find((point) => point.name == "ELW")
    ...
    if (active && payload && label) {
      return (
        <div className='border shadow-md bg-white margin-5 padding-10'>
          ...
          <p style={{ color: NORMAL_COLOUR }}>
            Normal: {payload[0].value}
          </p>
          <p style={{ color: UTILITY_COLOUR }}>
            Utility: {payload[1].value}
          </p>
        ...
        <ComposedChart>
          ...
          {tow && 
            <Scatter 
              name="TOW"
              dataKey="y"
              data={[tow]} fill={TOW_COLOUR} />
          }
          {elw && 
            <Scatter 
              name="ELW"
              dataKey="elw"
              data={[elw]} fill={ELW_COLOUR} />
          }
        </ComposedChart>
      </ResponsiveContainer>
